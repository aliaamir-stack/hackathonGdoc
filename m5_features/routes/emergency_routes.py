...
- POST /api/emergency/alert — Send Telegram GPS emergency alert
...
from m5_features.emergency.telegram_alert import telegram_alert_service
...
@router.post(
    ...
    summary="Send emergency Telegram alert with GPS",
    description=(
        "Sends an emergency alert message via Telegram bot "
...
async def send_alert(request: EmergencyAlertRequest) -> EmergencyAlertResponse:
    """
    Send emergency GPS alert via Telegram.
    ...
    try:
        result = await telegram_alert_service.send_emergency_alert(
